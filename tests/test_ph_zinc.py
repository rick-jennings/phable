...
def test_parse_zinc(zinc: str, expected: Grid):
    zinc_decoded = ph_from_zinc(zinc)

    assert isinstance(zinc_decoded, Grid)
    assert zinc_decoded == expected
    assert ph_to_zinc(expected) == zinc


def test_parse_nan():
    x = r"{a: NaN, b: -2.5min}"

    zinc_decoded = ph_from_zinc(x)
...
def test_list():
    ...
    zinc_decoded = ph_from_zinc(raw_zinc)
    ...
    assert zinc_decoded == expected
    assert ph_to_zinc(expected) == raw_zinc
