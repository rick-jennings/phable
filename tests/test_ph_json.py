...
def test_parse_list_of_dicts_to_json(
    ...
    assert ph_from_json(test_input) == expected
...
def test_grid_to_json_col1():
    ...
    cols = [
        kinds.GridCol("ts"),
        kinds.GridCol("v0", {"id": kinds.Ref("hisA")}),
        kinds.GridCol("v1", {"id": kinds.Ref("hisB"), "test_col_meta": kinds.Marker()}),
        kinds.GridCol("v2", {"id": kinds.Ref("hisC")}),
    ]
...
def test_create_single_his_write_grid():
    ...
    cols = [kinds.GridCol("ts"), kinds.GridCol("val")]
    rows = [
        {
...
def test_create_batch_his_write_grid():
    ...
    cols_haystack = [
        kinds.GridCol("ts"),
        kinds.GridCol("v0", {"id": kinds.Ref("hisA")}),
        kinds.GridCol("v1", {"id": kinds.Ref("hisB")}),
    ]
    rows_haystack = [
        {
            "ts": datetime.fromisoformat("2012-04-21T08:30:00-04:00").replace(
...
def test__parse_number():
    ...
    assert ph_from_json(z1) == kinds.Number(val=605.1, unit="kW")

    # successfully create a Number without unit
    z2 = {"_kind": "number", "val": "605.1"}
    assert ph_from_json(z2) == kinds.Number(val=605.1, unit=None)


def test__parse_marker():
    assert ph_from_json({"_kind": "marker"}) == kinds.Marker()


def test__parse_remove():
    assert ph_from_json({"_kind": "remove"}) == kinds.Remove()


def test__parse_na():
    assert ph_from_json({"_kind": "na"}) == kinds.NA()
...
def test__parse_ref():
    ...
    assert kinds.Ref("@foo", "Elec Meter") == ph_from_json(
        {"_kind": "ref", "val": "@foo", "dis": "Elec Meter"}
    )

    # successfully create a Ref without dis
    assert kinds.Ref("@foo") == ph_from_json(
...
def test__parse_date():
    ...
    assert ph_from_json({"_kind": "date", "val": "2011-06-07"}) == date(2011, 6, 7)
...
def test__parse_time():
    ...
    assert ph_from_json({"_kind": "time", "val": "14:30:00"}) == time(14, 30)
...
def test__parse_date_time():
    ...
    assert ph_from_json(a) == datetime.fromisoformat(a["val"]).replace(
        tzinfo=ZoneInfo("America/New_York")
    )
...
def test_parse_grid_col(json_input: dict[str, Any], expected: kinds.Grid) -> None:
    assert ph_from_json(json_input) == expected
